select r_category, sum(case when smw_max_voltage >= 33 then 1 else 0 end) "Subtransmission Poles"
...
and    nvl(smw_pole_order,1) = 1 -- Only get 1 record for S poles with duplicate E IDs
and    nvl(po_pole_order,1) = 1 -- Only get 1 record for multiple  physicals with the same parent site
